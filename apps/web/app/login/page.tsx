  ...
  return (
    <main className="flex-grow flex flex-col items-center justify-center px-8 pb-12 pt-12 md:pt-24 min-h-screen bg-[var(--color-surface)]">
      ...
      <div className="w-full max-w-md bg-[var(--color-surface-container-lowest)] rounded-[1.5rem] p-8 soft-shadow z-10">
        {/* Status messages */}
        {params.error && (
          <div id="login-error" className="mb-6 rounded-md bg-[var(--color-error-container)] p-4 text-sm font-medium text-[var(--color-on-error-container)]">
            {params.error}
          </div>
        )}
        {params.message && (
          <div id="login-message" className="mb-6 rounded-md bg-[var(--color-primary-fixed)] p-4 text-sm font-medium text-[var(--color-on-primary-fixed-variant)]">
            {params.message}
          </div>
        )}
        ...
        <form className="flex flex-col gap-6">
          ...
          <div className="group">
            <div className="flex justify-between items-center mb-2 px-1">
              <label htmlFor="password" className="text-[var(--color-on-secondary-container)] text-[13px] font-semibold">Password</label>
              <a href="#" className="text-[13px] font-medium text-[var(--color-on-secondary-container)] hover:text-[var(--color-primary)] transition-colors">Forgot Password?</a>
            </div>
            <div className="bg-[var(--color-surface-container-highest)] rounded-[1.5rem] px-6 py-4 transition-all duration-300 focus-within:bg-[var(--color-surface-container)] focus-within:ring-2 focus-within:ring-[var(--color-outline-variant)]">
              <input
                id="password"
        ...
        <div className="mt-8 pt-6 border-t border-transparent text-center">
          <p className="text-[var(--color-on-secondary-container)] text-sm">
            New to Fridge Manager? 
            <Link href="/signup" className="text-[var(--color-primary)] font-bold ml-1 hover:underline underline-offset-4 decoration-2">
              Create account
            </Link>
          </p>
        </div>
      ...
      <div className="hidden lg:grid grid-cols-2 gap-6 fixed left-12 bottom-12 w-64 opacity-20 pointer-events-none z-0">
        <div className="h-32 bg-[var(--color-primary-fixed)] rounded-[1.5rem]"></div>
        <div className="h-48 bg-[var(--color-secondary-fixed)] rounded-[1.5rem] mt-8"></div>
      </div>
    </main>
  )
}
